...
eventfile = "deposit_logs.csv"  # File to store logs
...
def log_event_to_csv(event):
    ...
    df.to_csv(eventfile, mode='a', header=not pd.io.common.file_exists(eventfile), index=False)
    print(f"Event logged to {eventfile}")
